...
from diia_auth_server.config import get_config, server_option
from diia_auth_server.database import db
from diia_auth_server.middlewares import sentry_middleware_factory, request_context_middleware
from diia_auth_server.routes import setup_routes
...
async def on_app_startup(app: web.Application):
    ...
    # Starting up...
    app.logger.info("Starting up diia_auth_server on address: {host}:{port}...".format(host=host, port=port))
    ...
    # Started!
    app.logger.info("diia_auth_server successfully started on {host}:{port}!".format(host=host, port=port))
...
async def on_app_cleanup(app: web.Application):
    ...
    """
    # Cleaning up...
    app.logger.info("Cleaning up diia_auth_server's resources...")

    # Clean up the database resources
    db.cleanup()

    # Cleaned up!
    app.logger.info("diia_auth_server's resources were successfully cleaned up!")
...
async def on_app_shutdown(app: web.Application):
    ...
    :param app: The application instance the signal is being executed for.
    """
    app.logger.info("Shutting down diia_auth_server...")
